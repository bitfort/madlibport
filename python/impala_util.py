...
def make_model_table(name):
  ...
  q += "INSERT INTO %s VALUES (0, encode_array(toarray(0,0,0)));" % name
  return q
...
def bismarck_inject_model(qry, model_table):
  return qry.replace('__PREV_MODEL__', 'decode_array(%s.model)' % model_table)
...
def bismarck_epoch(model_table, data_table, uda_gen, epoch, helper_col):
  ...
  q = 'INSERT INTO %(hist)s SELECT %(epoch)d, encode_array(%(uda)s) FROM %(hist)s, %(dat)s ' \
