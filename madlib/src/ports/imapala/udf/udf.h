...
// Note: be very careful about including Impala internal headers.
// We want a clear separation between internal and external APIs.
// This should be the only header that UDF/UDAs need to include.
...
struct StringVal : public AnyVal {
  ...
  // be used to return StringVal objects in UDF/UDAs that need to new strings.
...
class UdfContext {
 public:
  ...
  // Returns the type of the ith input argument (0 indexed).
  ...
  // overflow or other similar recoverable error conditions.
  ...
  // Frees buffers returned from Allocate()
...
// Optional function that is called once before any calls to the Udf to
// evaluate values. This is the appropriate time for the Udf to validate
...
// a typedef as the actual UDF's signature is dynamic.
//    typedef <*Val> Evaluate(UdfContext* context, <type* args>);
// The UDF must return one of the *Val structs. The UDF must accept a pointer
// to a UdfContext object and then a pointer for each of the input arguments.
...
// input value.
// TODO: we can't implement string_concat with this interface, since that takes two
// input arguments. e.g. "string_concat(string_col, separator)". Does this generally
// make sense or can we have the separator be passed in as an external constant.
