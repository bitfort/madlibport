...
#ifndef MADLIB_mainmem_ANYTYPE_IMPL_HPP
#define MADLIB_mainmem_ANYTYPE_IMPL_HPP
...
inline void * AnyType::getUserFuncContext(){
        return NULL; //this->mSysInfo->user_fctx;
}
...
inline MemoryContext AnyType::getCacheMemoryContext() {
    return NULL; //this->mSysInfo->cacheContext;
}
 ...
 * function.
 * This constructor immediately converts the object into a mainmemQL Datum
...
AnyType::operator[](uint16_t inID) const {
    ...
    if (mContentType == FunctionComposite) {
        ...
        if (inID == 0) {
            ...
            // memory is dangerous.
            // See warning at:
            // http://www.mainmemql.org/docs/current/static/xfunc-c.html#XFUNC-C-BASETYPE
    ...
    */
    /* if (mContentType == NativeComposite) */ 
    // {
...
/**
 * @brief Return a mainmemQL Datum representing the current object
 *
 ...
 * memory.
 *
 * @param inFnCallInfo The mainmemQL FunctionCallInfo that was passed to the
 ...
 * @param inTargetTypeID mainmemQL OID of the target type to convert to. If
...
/**
 * @brief Convert values to mainmemQL Datum in AnyType constructor (or only
 *     when needed)?
 *
 ...
 * mainmemQL Datum type, and it is better to only lazily convert to Datum
 * (i.e., only when needed by getAsDatum()).
 *
 * Since mainmemQL is single-threaded, it is sufficient to maintain a global
